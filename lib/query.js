 ...
 *
 * 可传入多个参数
 * 若i-th参数为字符串，则直接解析成 ai
 * 若i-th参数为数组，则聚合解析 ai2 ? ai0(ai1) : ai0(`ai1`)
 */
AAquery.prototype.field = function() {
  ...
  for (var i = 0; i < arg.length; i++) {
    ...
    if (typeof arg[i] == 'object')
      this.sql.field += arg[i][0] + '(' + (arg[i][2] ? arg[i][1] : mescapeId(arg[i][1])) + ')';
    else
...
// 其他选项
AAquery.prototype.option = function(option) {
  this._option = option;
  return this;
}
